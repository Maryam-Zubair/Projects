...
import certifi
# SSL Certificate issue I phase on Mac
...
def upload_url(file_name = url_file_name):
    try:     
        data = request.get_json()
        url = data.get('url')
...
def signup():
    if request.method == 'POST':
        ...
        # Call the signup function from authentication.py
        user_created = authentication.signup(email, name, password)
        if user_created:
...
def login():
    if request.method == 'POST':
        email = request.form.get('email')
        password = request.form.get('password')
        user_email, user_name = authentication.login(email, password)
        if user_email:
            session['user_email'] = user_email  # Set user email in session
            # You can also store the user's name in the session if needed
...
def logout():
    ...
    # on hitting logout, user will recieve an email
    if user_email:
        llm.send_translated_email(user_email, llm.language)
